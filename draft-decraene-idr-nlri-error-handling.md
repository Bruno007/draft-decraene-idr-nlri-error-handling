...
The ordering of NLRIs within the Treat-As-Withdraw MUST be the same as their ordering within the corresponding MP_REACH_NLRI. <!-- see my email for discussion about why I added this. We only need it if we keep the text mandating the comparison of the two attributes. -->

If the AFI/SAFI specification allows for different NLRI encodings in the MP_UNREACH_NLRI, the simplest format MUST be used. For example, {{RFC3107}} allows the use of either the MPLS label stack originally sent or the static 0x800000 value. The latter is simpler in that the size is smaller and the number of labels to parse is reduced.
...
<!-- Furthermore, in some implementations it may be the case that a different code path is used to generate the MP_UNREACH_NLRI encoding than is used to generate the MP_REACH_NLRI encoding. -->
